...
int main(void) {
    ...
    tusb_init();
    // Start HIDDEN — wait for WiFi to tell us when to show
    bt_state = 1;
    printf("Start HIDDEN (waiting for WiFi)\r\n");
    ...
    while (1) {
        ...
        if (!trigger && prev_trigger && pulse_start_ms != 0) {
            ...
            if (pulse_len > PULSE_THRESHOLD_MS) {
                ...
                // Short pulse = DISCONNECT (hide BT for 10s)
        ...
        // If disconnected, count down
        if (bt_state == 1) {
            uint32_t start = to_ms_since_boot(get_absolute_time());
            while ((to_ms_since_boot(get_absolute_time()) - start) < DISCONNECT_MS) {
                tud_task();
                sleep_ms(10);
                // Check for new pulse during disconnect
                trigger = gpio_get(TRIGGER_PIN);
                if (trigger && !prev_trigger) {
                    pulse_start_ms = to_ms_since_boot(get_absolute_time());
                }
                if (!trigger && prev_trigger && pulse_start_ms != 0) {
                    uint32_t pl = to_ms_since_boot(get_absolute_time()) - pulse_start_ms;
                    if (pl > PULSE_THRESHOLD_MS) {
                        // Long pulse during disconnect = reconnect early
                        printf("Early CONNECT\r\n");
                        tud_connect();
                        bt_state = 0;
                        pulse_start_ms = 0;
                        break;
