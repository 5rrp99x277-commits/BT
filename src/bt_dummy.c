/*
  ...
  Logic:
    ...
    - Short pulse (< 50ms) on GPIO3 -> disconnect (hide)
    - Long pulse (> 50ms) on GPIO3 -> connect (show BT)
    - Stay hidden until long pulse — no auto-reconnect
  
  Board: RP2040-Zero (WS2812 LED on GPIO16)
...
#define WS2812_PIN          16   // RP2040-Zero onboard RGB LED

// Simple WS2812 bit-bang driver
...
// Set LED color: hidden=RED, visible=GREEN
static inline void ws2812_set(bool visible) {
    if (visible) {
        ws2812_put_pixel(0x00FF00);  // GREEN = visible
    } else {
        ws2812_put_pixel(0xFF0000);  // RED = hidden
    }
}
...
uint8_t const desc_fs_configuration[] = {
    ...
    0, 0, 2,
    0x08, 0x06, 0x50,  // MSC / SCSI / Bulk-only
    0,
...
int main(void) {
    ...
    tusb_init();
    tud_disconnect();  // CRITICAL: start HIDDEN immediately
    bt_state = 1;
    ws2812_set(false);  // LED RED = hidden
    printf("Start HIDDEN (WiFi controls visibility via pulses)\r\n");
    ...
    while (1) {
        ...
        if (!trigger && prev_trigger && pulse_start_ms != 0) {
            ...
            if (pulse_len > PULSE_THRESHOLD_MS) {
                ...
                if (bt_state == 1) {
                    tud_connect();
                    bt_state = 0;
                    ws2812_set(true);  // LED GREEN = visible
                }
            } else {
        ...
        // Stay hidden forever. Only long pulse (>50ms) can show BT again.
